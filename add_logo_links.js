...
function addLogoLink(filePath) {
    if (!filePath.endsWith('.html')) return;

    let content = fs.readFileSync(filePath, 'utf-8');

    // 查找logo部分并添加链接
    const logoPattern = /<div class="logo">[\s\S]*?<svg class="logo-icon"[\s\S]*?<\/svg>[\s\S]*?<span class="logo-text">Veck<span class="logo-io">\.io<\/span><\/span>[\s\S]*?<\/div>/;
...
console.log('\nLogo links added to all secondary pages!');